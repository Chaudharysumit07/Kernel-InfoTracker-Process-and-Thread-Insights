...
** Function Definition
*/
static int find_max_stack(void)
{
    ...
    {
        if (task->mm && task->real_parent->pid== current->real_parent->pid)
        {
            unsigned long stack_size = task->mm->stack_vm;
...
{ 
	...
	case 0:
        ...
        case 4:
            ...
            break;
         case 5:
             len = sprintf(buf, "%d", count_threads(current->pid));
            break;
         case 6:
             len = sprintf(buf, "%d", count_open_files());
            break;
         case 7:
             len = sprintf(buf, "%d", find_max_stack());
            break;
        default:
            len = sprintf(buf, "%d", -EINVAL);
    }
